<%-- 
    Document   : formularioFV
    Created on : 11/10/2023, 7:41:46 a. m.
    Author     : camil
--%>
...
<html>
    <head>
        ...
        <title>Formulario Factura Venta</title>
    </head>
    ...
    <jsp:useBean id="unC" class="modelo.Clientes" scope="request" />
    ...
    <body class="miP">
        <jsp:include page="jspf/menu.jspf"></jsp:include>
            ...
            <div class="miDiv">
                <center class="miC"><h1 class="nt">Formulario Factura Venta</h1></center> 
                ...
                <div class=" mit table-responsive ">
                    ...
                    <table border="1" class="table table-hover table-striped table-bordered ">
                        ...
                        <form action="ControladorFV" method="post">
                            <td><input type="hidden" name="fIdFV" value="${laFV.idFV}">
                                <input type="number" name="fDtoF" value="${laFV.dtoF}"></td>
                            <td><input type="date" name="fFechaF" value="${laFV.fechaF}"></td>
                            <td><input type="number" name="fTotalF" value="${laFV.totalF}"></td>
                            <td>
                                <select name="fIdAdmin">

                                    <c:forEach items="${unAdmin.listar(0)}" var="elAdmin">
                                        <option value="${elAdmin.idAdmin}" <c:if test="${elAdmin.idAdmin == laFV.idAdmin}" >
                                                Selected
                                            </c:if>>${elAdmin.nombre}                                    
                                        </option>
                                    </c:forEach> 
                            ...
                            <td>
                                <select name="fIdC">
                                    ...
                                    <c:forEach items="${unC.listar(0)}" var="elC">
                                        <option value="${elC.idC}" <c:if test="${elC.idC == laFV.idC}" >
                                                Selected
                                            </c:if>>${elC.nombre}                                    
                                        </option>
                                    </c:forEach> 
                    ...
                    <tr>
                    <form action="ControladorFV" method="post">
                        <td><input type="hidden" name="fIdFV" value="0">
                        ...
                        <td>
                            <select name="fIdAdmin">
                                ...
                                <c:forEach items="${unAdmin.listar(0)}" var="elAdmin">
                                    <option value="${elAdmin.idAdmin}" <c:if test="${elAdmin.idAdmin == laFV.idAdmin}">selected</c:if>>${elAdmin.nombre}</option>
                                </c:forEach>
                            </select>
                        </td>
                        <td>
                            <select name="fIdC">
                                <option value="">Selecciona Cliente</option> <!-- Opción fija -->
                                <c:forEach items="${unC.listar(0)}" var="elC">
                                    <option value="${elC.idC}" <c:if test="${elC.idC == laFV.idC}" >Selected</c:if>>${elC.nombre}</option>
                                </c:forEach> 
                            </select>
                        </td>
